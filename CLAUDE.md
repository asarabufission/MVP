...
### 2026-05-06 — Prototype V2 walkthrough → SPEC v4.1 amendments

After the May 6 stakeholder walkthrough (Rakesh, Mishtu, Satya, Sita, Sandeep), SPEC.md was amended with v4.1 changes. Anyone starting a new phase should re-read the affected sections rather than relying on prior memory:

- **§3 (Scope)** — IN: edit datasource / credential rotation, server-side client search, dashboard navigability + inline failed-run errors, MSP-level Client Identifier Column picker, async generate-and-notify reports with `/reports/history`. OUT: NoSQL pre-computed report cache (post-MVP), on-prem packaging (post-MVP).
- **§7 (Datasources)** — added `PATCH /datasources/{id}` (metadata) and `POST /datasources/{id}/rotate-credentials` (new Secrets Manager *version* under existing `secret_arn`; rotate endpoint also performs the test connection — there is no separate dry-run).
- **§7 (Clients)** — `GET /clients` now takes `?search=&limit=&cursor=` and returns `{items, nextCursor, totalCount}` (cursor-paginated). Required by `ClientSearchSelect` for MSPs with 1,000+ clients (LeafTech: 1,332).
- **§9 Step 3** — when scope is `MSP_LEVEL`, render a dedicated highlighted **Client Identifier Column** picker above the standard-fields table.
- **§11** — new route `/reports/history`; new `useReportsStore` for cross-page polling; ClientSourceMappingPage left pane is a `ClientSearchSelect` combobox (not a list); DatasourcesPage gains per-row Edit; DashboardPage stat cards are clickable nav targets and the failed-runs alert deep-links to `/job-runs?status=FAILED&focusId=<uuid>` with inline error rendering.

Phase mapping for the new work:
- Phase 3 (wizard backend) — verify `customParameters` flow through Lambda + Phase-2 activation refetch (already specced; just confirm).
- Phase 4 (clients + assignments) — implement `?search=&limit=&cursor=` on `GET /clients`.
- Phase 6 (frontend pages) — DashboardPage navigability, JobRunsPage inline errors, DatasourcesPage Edit modal + `ClientSearchSelect` reuse, ClientSourceMappingPage picker.
- Phase 7 (wizard frontend) — Step 3 MSP-level Client Identifier Column picker.
- Phase 8 (reports) — split `ReportsPage` (form) from `ReportsHistoryPage` (list), wire `useReportsStore` polling and topbar notification, add `POST /datasources/{id}/rotate-credentials` and corresponding `EditDatasourceModal`.
